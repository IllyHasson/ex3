...
import { all, append, chain, concat, equals, map, sort, uniq, zip, filter, includes, is, differenceWith } from "ramda";
...
const flattenUnion = (tes: TExp[]): TExp[] =>
    (tes.length > 0) ?
        (isUnionTExp(tes[0]) || isInterTExp(tes[0])) ? [...tes[0].components, ...flattenUnion(tes.slice(1))] :
...
const flattenInter = (tes: TExp[]): TExp[] => {
    const union =
        (tes.length > 0) ?
            (isUnionTExp(tes[0]) || isInterTExp(tes[0])) ? [...tes[0].components, ...flattenInter(tes.slice(1))] :
                [tes[0], ...flattenInter(tes.slice(1))] :
            [];
    return returnDuplicatesAndNever(union);
}
